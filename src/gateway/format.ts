/**
 * Request body format shaping for the gateway forward path.
 *
 * Owns everything that rewrites the OUTBOUND request body:
 * - `stripOcPrefix` / `rewriteModelBody`: gateway ids carry `oc/`, upstream
 *   bare ids do not. Only the prefix is stripped.
 * - `sanitizeResponsesBody`: drops reasoning blocks + previous_response_id
 *   (egress-bound, replay through another IP is rejected upstream).
 * - `translateChatToResponses`: chat/completions → Responses allowlist
 *   translation for chat-only clients hitting spark models.
 *
 * Leaf module: no imports from gateway internals. `forward.ts` builds route
 * resolution + passthrough on top and re-exports these symbols.
 */
